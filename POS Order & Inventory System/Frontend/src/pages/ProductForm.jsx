...
export default function ProductForm({ onSubmit, onCancel, product }) {
	...
	const submit = async event => {
		...
		const stockNum = Number(form.availableStock);
		if (!form.name.trim() || isNaN(priceNum) || priceNum < 0 || isNaN(stockNum) || stockNum < 0) {
			return setError('Enter a valid name, non-negative price, and non-negative stock.');
		}
		try {
			await onSubmit({ name: form.name.trim(), price: priceNum, availableStock: stockNum });
